...
//Rare exceptions to indicate a bug in the _goapplication code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_goapplication = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_goapplication]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.goapplication.module"
